...
shinyServer(function(input,output,session){
  ...
  observe({
    if(!is.null(original.data())){
      updateSelectInput(session, "seqSelect",
                        choices = colnames(original.data()))
    }
  })

  observeEvent(input$simButton, {
    seq = as.character(values$fulldata[,input$seqSelect])
    data = values$fulldata
    
    seq.excl.start = str_remove_all(input$seq.excl.start, " ")
    seq.excl.end = str_remove_all(input$seq.excl.end, " ")
    
    seq.start.with = str_remove_all(input$seq.start.with, " ")
    seq.end.with = str_remove_all(input$seq.end.with, " ")
    
    if(seq.excl.start != ""){
      seq.temp = str_sub(seq, start = 1, end = str_length(seq.excl.start))
      seq.temp = which(seq.temp == seq.excl.start)
      seq[seq.temp] = str_sub(seq[seq.temp], start = (str_length(seq.excl.start) + 1), end = str_length(seq[seq.temp]))
      data[,input$seqSelect] = seq
    }
    
    if(seq.excl.end != ""){
      seq.temp = str_sub(seq, start = (str_length(seq) - str_length(seq.excl.end) + 1), end = str_length(seq))
      seq.temp = which(seq.temp == seq.excl.end)
      seq[seq.temp] = str_sub(seq[seq.temp], start = 1, end = (str_length(seq) - str_length(seq.excl.end)))
      data[,input$seqSelect] = seq
    }
    
    if(seq.start.with != ""){
      seq.temp = str_sub(seq, start = 1, end = str_length(seq.start.with))
      seq.temp = which(seq.temp == seq.start.with)
      seq = seq[seq.temp]
      data = data[seq.temp, ]
    }

    if(seq.end.with != ""){
      seq.temp = str_sub(seq, start = (str_length(seq) - str_length(seq.end.with) + 1), end = str_length(seq))
      seq.temp = which(seq.temp == seq.end.with)
      seq = seq[seq.temp]
      data = data[seq.temp, ]
    }
    
    if(input$seq.start > 0 & input$seq.end >= input$seq.start){
      seq.temp = str_sub(seq, start = input$seq.start, end = input$seq.end)
      seq.temp = str_split(seq.temp, "")
      
      seq.accepted = unlist(str_split(input$seq.accepted, ""))
      
      seq.temp = lapply(seq.temp, function(x, temp){ return(sum(x  %in% temp)) }, seq.accepted)
      
      seq.temp = which(seq.temp == (input$seq.end - input$seq.start + 1))
  ...
  # mstValues <- reactiveValues(edges = NULL, 
  #                             centroids = NULL,
  #                             clusters = NULL,
  #                             flag2 = NULL,
  #                             legenddf = NULL,
